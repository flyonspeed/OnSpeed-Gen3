...
|---|---|---|---|
| Audio harness | `tools/audio-sweep/engines/master/harness.cpp` | EDIT | Add `--frames-stdin` mode: parse `#1` frames from stdin, derive `(aoa, flap_thresholds)` per frame, drive the existing `ToneCalc → AudioMixer` pipeline. Existing `--duration --aoa-start --aoa-end` sweep mode preserved. |
