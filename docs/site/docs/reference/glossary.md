...
| **Ram Rise** | The temperature rise at an OAT probe due to kinetic heating of the airstream brought to rest at the probe surface. Approximately `K · 0.2 · M² · TAT_K` in Kelvin. A few degrees Celsius at typical GA cruise speeds. See **Recovery Factor (K)** in this table. |
...
| **SAT** | *Static Air Temperature.* The freestream air temperature without the ram-rise component. Used by OnSpeed for TAS and density-altitude computations. See **TAT** and **Recovery Factor (K)** in this table. |
...
| **TAT** | *Total Air Temperature.* What an OAT probe physically reads in flight — freestream temperature plus ram rise. Pilots see TAT on the panel. OnSpeed converts TAT to SAT internally; see **Recovery Factor (K)** in this table. |
