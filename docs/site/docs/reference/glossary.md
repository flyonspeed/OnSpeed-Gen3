...
| **Ram Rise** | The temperature rise at an OAT probe due to kinetic heating of air brought to rest at the probe surface. Approximately `K · 0.2 · M² · TAT_K` in Kelvin: a few degrees at GA cruise speeds, tens of degrees on transports. The probe reads **TAT** (warmer); the freestream air is at **SAT** (cooler). |
| **Recovery Factor (K)** | A probe-specific constant in `[0, 1]` for the fraction of theoretical ram rise the probe physically captures. A bare thermistor captures ~75% (`K = 0.75`); a properly shielded TAT probe captures ~100% (`K = 1.0`). Config parameter: `OAT_RECOVERY_FACTOR`. See [advanced configuration](../configuration/advanced.md#oat-recovery-factor-k). |
| **SAT** | Static Air Temperature — the freestream air temperature without the ram-rise component. What density-altitude and TAS calculations physically depend on. OnSpeed derives SAT from the probe's **TAT** reading via the **Recovery Factor (K)** correction. |
...
| **TAS** | True Airspeed — IAS corrected for air density (altitude and temperature). OnSpeed corrects for ram-rise before computing TAS using the **Recovery Factor (K)** to convert **TAT** to **SAT**. |
| **TAT** | Total Air Temperature — what an OAT probe physically reads in flight. Equals freestream **SAT** plus a ram-heating component from kinetic energy at the probe. Pilots see TAT on the panel; OnSpeed converts TAT to SAT internally before computing density altitude and TAS. |
