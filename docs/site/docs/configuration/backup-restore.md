...
2. **SD card** (`config.cfg`) — if present and load succeeds, becomes the active config
3. **Flash memory** (LittleFS backup) — only consulted if the SD card has no config file (or the SD load failed)

The SD card is the authoritative source. The flash copy is a backup that the firmware writes after every successful SD save, so the two are normally identical. The flash copy only matters if the SD card has no `config.cfg` (fresh card, post-format, or card swapped in from another box) — in that case the firmware boots from the flash backup so you keep your last saved settings.
...
- Delete the `config.cfg` file from the SD card (next boot loads from flash backup, or compiled defaults if flash is also empty)
