...
- **Turbulence** — in rough air, AOA readings fluctuate. The system applies smoothing, but strong turbulence will cause tone variations.
...
OnSpeed provides **awareness**, not prevention. It tells you where you are relative to stall — it cannot prevent a stall. Proper pilot technique is always required.
...
2. **Verify against known speeds** — after calibration, confirm that the "on speed" tone corresponds to your known approach speed for each flap configuration.
