...
export function run(assert) {
  ...
  // Counts: short ticks for i in -85..+85 step 10 (excluding 0), so 17;
  // long ticks for i in -90..+90 step 10 (excluding 0), so 18.
  // Each long tick also has a text label.
  const lines = svg.querySelectorAll('line');
  const texts = svg.querySelectorAll('text');
  assert.equal(lines.length, 17 + 18, '17 short + 18 long ticks');
  ...
  const longTicks = Array.from(lines).slice(17);  // skip 17 short
  // i=10 is the 10th in -90..-10, 10..90 sequence after dropping 0.
  // Sequence: -90 -80 -70 -60 -50 -40 -30 -20 -10 10 20 30 40 50 60 70 80 90.
  // Index of 10 is 9 (0-based).
