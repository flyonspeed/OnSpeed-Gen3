...
The web bundle ships pure-JS tests that run on Node 20+ with zero deps. CI runs these in the `web-tests` job — same set as below — so a forgotten regression breaks the PR check:
...
If you also touched a legacy form page (`/aoaconfig` or `/sensorconfig`), include the corresponding sources under `tools/web/legacy-pages/`.
...
- `software/OnSpeed-Gen3-ESP32/Web/legacy_pages.h/.cpp` — generated `/aoaconfig` + `/sensorconfig` template strings. Same.
