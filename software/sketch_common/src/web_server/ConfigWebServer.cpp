...
#include "Web/sim_index_js.h"
#include "Web/sim_index_wasm.h"
...
void HandleIndexer();
void HandleSimIndexJs();
void HandleSimIndexWasm();
...
void CfgWebServerInit()
    ...
    CfgServer.on("/sim/index.js",    HTTP_GET,  HandleSimIndexJs);
    CfgServer.on("/sim/index.wasm",  HTTP_GET,  HandleSimIndexWasm);
...
// ----------------------------------------------------------------------------
// Indexer tab handlers.
//
// /indexer        : tablet-friendly HTML shell that loads /sim/index.js
//                   (which fetches /sim/index.wasm), then opens the live
//                   WebSocket on port 81 and pipes binary #1 frames into
//                   the WASM via the exported _inject_serial_byte symbol.
// /sim/index.js   : Emscripten JS glue for the M5 sim live-data build,
//                   shipped gzipped from PROGMEM.
// /sim/index.wasm : Compiled M5 sim live-data WebAssembly module,
//                   shipped gzipped from PROGMEM.
//
// Both /sim/* assets are gzipped at build time and served with the
// Content-Encoding: gzip header so the browser inflates on the fly.
// ETag uses BuildInfo::version, so a firmware OTA invalidates the cache.
...
// Helper: send a gzipped binary blob from PROGMEM with the appropriate
// Content-Encoding header.  Same ETag conditional-GET path as the rest
// of the static assets — version-bound so any firmware change triggers
// a full re-download (avoids stale WASM after an OTA).
static void SendGzippedFromProgmem(const uint8_t* data, size_t length, const char* contentType)
    {
    if (SendWithETag(contentType))
        return;
    CfgServer.sendHeader("Content-Encoding", "gzip");
    CfgServer.setContentLength(length);
    CfgServer.send(200, contentType, "");
    // sendContent_P with explicit length handles binary (NUL-byte
    // tolerant). reinterpret_cast: the WebServer API takes PGM_P
    // (const char*) but the underlying read is byte-wise.
    CfgServer.sendContent_P(reinterpret_cast<PGM_P>(data), length);
    }

void HandleSimIndexJs()
    {
    SendGzippedFromProgmem(kSimIndexJs_gz, kSimIndexJs_gz_len, "application/javascript");
    }

void HandleSimIndexWasm()
    {
    SendGzippedFromProgmem(kSimIndexWasm_gz, kSimIndexWasm_gz_len, "application/wasm");
    }
