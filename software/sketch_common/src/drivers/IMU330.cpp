...
void IMU330::Init()
{
  ...
  // STRESS-BRANCH ONLY: ODR_XL[3:0] = 0110 = 416 Hz (next LSM6 step
  // above the production 0101 = 208 Hz). Range and LPF bits unchanged.
  SensorSPI->WriteRegByte(uChipSel, IMU_WRITE_ADDR(CTRL1_XL), 0b01101100); // 416hz ODR, +/-8G, LPF2 disabled
  delay(50);

  // enable gyroscope
  // STRESS-BRANCH ONLY: ODR_G[3:0] = 0110 = 416 Hz, full-scale unchanged.
