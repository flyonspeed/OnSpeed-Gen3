...
// buffer of ±kSynthHalfWindow rows. step() returns an empty optional until
// kSynthHalfWindow rows have been fed (the "not ready" lag); after that it
// returns the synth for the row kSynthHalfWindow ticks in the past. flush()
// drains the remaining buffered rows after the input stream ends.
//
...
// Synth ADC constants
//
// kSynthHalfWindow: number of rows on each side of a transition where the
// smoothstep painter touches the output. Two seconds at 50 Hz = 100 rows.
// The circular buffer holds 2*kSynthHalfWindow+1 rows.
//
...
// ============================================================================
inline constexpr int kSynthHalfWindow = 100;  // ±2 sec at 50 Hz, ~20 KB
...
//   step(row) returns empty optional for the first kSynthHalfWindow calls
//   (the buffer is filling); thereafter returns the synth for the row
//   kSynthHalfWindow ticks in the past.
//   flush() returns up to kSynthHalfWindow results for the final rows.
...
public:
    ...
    //   - Returns empty for the first kSynthHalfWindow calls while the
    //     circular buffer fills.
    //   - Thereafter returns the synth for the row kSynthHalfWindow ticks
    //     in the past. Both edges of any detent transition are visible in
    //     the ±kSynthHalfWindow window; the smoothstep paints correctly.
    std::optional<ReplayStepResult> step(const onspeed::LogRow& row);

    // Drain remaining buffered rows after the input stream ends.
    //
    // When flapsRawAdcAvailable was false, returns up to kSynthHalfWindow
...
private:
    ...
    // (with a placeholder flapsRawAdc). The buffer holds 2*kSynthHalfWindow+1
    // entries. When it's full, the oldest entry (at offset emitIdx_) is the
    // one kSynthHalfWindow ticks in the past and is ready to emit after the
    // smoothstep is applied.
    //
    // bufHead_: next slot to write in circBuf_ (wraps mod kBufSize).
    ...
    // to remember transitions within the current ±kSynthHalfWindow window.
    // -------------------------------------------------------------------------
    // The buffer needs kSynthHalfWindow + 1 slots:
    //   - kSynthHalfWindow rows already buffered (but not yet emitted)
    //   - 1 slot for the new row being written
    // When bufSize_ reaches kSynthHalfWindow + 1, the oldest entry is
    // exactly kSynthHalfWindow ticks behind the write point → emit it.
    static constexpr int kBufSize = kSynthHalfWindow + 1;
    ...
    ReplayStepResult circBuf_[kBufSize];

    // Index of the oldest entry in circBuf_ (next slot to read for emission).
    int bufHead_    = 0;
    // Number of valid entries currently buffered (0..kBufSize).
    int bufSize_    = 0;
    ...
    // simultaneous transitions can sit in the ±kSynthHalfWindow window (in
    // practice ≤1 at any time for typical flight).
    ...
    // pushed). Uses all stored transitions_ within the ±kSynthHalfWindow window.
    ...
    // Returns the entry that is now ready for emission (kSynthHalfWindow ticks
