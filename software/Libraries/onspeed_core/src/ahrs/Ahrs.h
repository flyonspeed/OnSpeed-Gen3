...
struct AhrsConfig {
    ...
    // Probe recovery factor for the ram-rise SAT correction. Range [0, 1].
    // 0 disables (TAS uses raw TAT, no correction).
    // 0.75 default = typical bare/exposed thermistor GA install.
    // See sensors/SatCorrect.h.
