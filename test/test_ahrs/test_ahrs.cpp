...
// EKFQ vertical-channel covariance reset on the iasAlive false→true edge.
//
// In EKFQ mode the iasWasBelowThreshold_ rising edge invokes
// `EKFQ::resetVerticalCovariance(baro_z)`, which zeros all rows and
// columns of P that touch (z, vz, b_az), reseeds their diagonal entries
// from the initial covariance, and re-seeds the state z from the
// current baro reading. The purpose is to clear cross-correlations
// that built up while the baro path was effectively gated on the
// ground, letting the filter re-learn the vertical channel without
// disturbing attitude or gyro-bias states.
//
// Verifies the reset path does not NaN the filter and downstream
// pitch / roll / derived-AOA outputs stay finite across the edge.
void test_ekfq_vertical_covariance_reset_on_ias_alive_rising_edge(void)
{
    AhrsConfig cfg = makeCfg(Algorithm::Ekfq);
    Ahrs a{cfg};
    AhrsInputs in = levelSeed();
    ...
    // Cross the iasAlive gate — this triggers the vertical-channel reset.
...
int main(void)
{
    ...
    RUN_TEST(test_ekfq_vertical_covariance_reset_on_ias_alive_rising_edge);
