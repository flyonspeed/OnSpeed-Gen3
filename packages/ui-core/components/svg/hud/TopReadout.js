...
// Black-fill rounded rect with white border; label small + value large.
//
...
export const HudTopReadout = ({ label, value, x, y, width, height }) => {
  ...
  return html`
    <g data-widget=${`hud-top-${label.toLowerCase()}`}>
      ...
      <text x=${x + H.HUD_TOP_LABEL_PAD_X} y=${y + h / 2}
      ...
      <text x=${x + w - H.HUD_TOP_VALUE_PAD_X} y=${y + h / 2}
            font-family="'B612', 'Helvetica Neue', Arial, sans-serif"
            font-weight="bold"
            font-size=${H.HUD_TOP_VALUE_FONT_SIZE}
            fill="var(--white)"
            text-anchor="end" dominant-baseline="central">${value}</text>
    </g>`;
};
